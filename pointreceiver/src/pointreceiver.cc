...
static moodycamel::BlockingReaderWriterCircularBuffer<PointCloud> cloud_queue(1);
		...
		if (group == "live") 
			...
			if (header_msg == "clear") { 
				...
				if (frame_time - last_snapshot_time > 1000ms) {
					...
					//synthesized_snapshot_frames = std::reduce(snapshot_frames.begin(), snapshot_frames.end(), 
							//PointCloud{}, [](auto a, auto b) -> PointCloud { return a + b; });
