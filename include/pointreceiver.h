 ...
 *
 * This structure encapsulates a pointer to an array of 2D float vector values
 * and the number of elements in the array.
 */
typedef struct {
  pointreceiver_float2_t
      *data;    /**< Pointer to an array of 3D float vector values */
 ...
 *
 * This structure encapsulates a pointer to an array of 3D float vector values
 * and the number of elements in the array.
 */
typedef struct {
  pointreceiver_float3_t
 ...
 *
 * This structure encapsulates a pointer to an array of 4D float vector values
 * and the number of elements in the array.
 */
typedef struct {
  pointreceiver_float4_t
 ...
 */
PR_EXPORT bool
pointreceiver_free_sync_message(pointreceiver_context *ctx,
                                pointreceiver_sync_message *out_message);
 ...
 *
 * Registers interest in the channel identified by @p address. The
 * receiver thread applies the subscription to its SUB socket,
 * which propagates upstream to the publisher. Matching is exact (a
 * NUL separator is appended internally, so "foo" will not also
 * match "foobar"). NULL or "" subscribes to all channels. Safe to
 * call from any thread and before the receiver is started; pending
 * subscriptions are applied once it is running.
 *
 * @param ctx Pointer to the PointReceiver context.
 * @param address Channel address, or NULL/"" for all channels.
 * @return 0 on success, non-zero on failure.
 */
PR_EXPORT int pointreceiver_subscribe_to_point_cloud(pointreceiver_context *ctx,
                                                     const char *address);
...
/**
 * @brief Dequeues the freshest pending frame across all subscribed channels.
 *
 * Waits up to @p timeout_ms for any subscribed channel to have a new frame,
 * then deserialises and returns the most recent frame for one channel,
 * reporting that channel's address. Frames superseded within a channel before
 * dequeue are discarded without being deserialised. The returned buffer
 * pointers remain valid until the next call on the same context.
 *
 * @param ctx Pointer to the PointReceiver context.
 ...
 * @param timeout_ms Timeout in milliseconds to wait for a frame.
 * @return true if a frame was dequeued, false on timeout or decode failure.
 */
PR_EXPORT bool pointreceiver_dequeue_point_cloud(
    pointreceiver_context *ctx, char *out_address, int address_capacity,
    pointreceiver_pointcloud_frame *out_frame, int timeout_ms);

/**
 * @brief Returns the number of channel addresses observed so far.
 *
 * Only channels that have delivered a frame (i.e. ones currently subscribed)
 * are counted. Intended for populating a discovery list.
 *
 * @param ctx Pointer to the PointReceiver context.
 * @return Number of known channel addresses.
 */
PR_EXPORT size_t pointreceiver_known_address_count(pointreceiver_context *ctx);

/**
 * @brief Retrieves a known channel address by index.
 *
 * Addresses are returned in stable sorted order. @p index must be less than
 * pointreceiver_known_address_count().
 *
 * @param ctx Pointer to the PointReceiver context.
 * @param index Zero-based index of the address.
 * @param[out] out Buffer receiving the NUL-terminated address.
 * @param out_cap Size in bytes of @p out.
 * @return true if written, false if index is out of range.
 */
PR_EXPORT bool pointreceiver_get_known_address(pointreceiver_context *ctx,
