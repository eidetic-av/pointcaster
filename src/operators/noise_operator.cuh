...
using pc::types::position;
  ...
  __device__ indexed_point_t NoiseOperator::operator()(indexed_point_t point) const {
    ...
    pos.x += cudaNoise::repeaterSimplex({pos_f.x, pos_f.y, pos_f.z}, scale,
                                        seed, repeat, lacunarity, decay) *
             magnitude;
    pos.y += cudaNoise::repeaterSimplex({pos_f.z, pos_f.x, pos_f.y}, scale,
                                        seed, repeat, lacunarity, decay) *
             magnitude;
    pos.z += cudaNoise::repeaterSimplex({pos_f.y, pos_f.z, pos_f.x}, scale,
                                        seed, repeat, lacunarity, decay) *
             magnitude;
