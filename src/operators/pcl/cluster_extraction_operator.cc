...
namespace pc::operators::pcl_cpu {
		...
		_host_thread = std::jthread([this, max_concurrency](std::stop_token st) {
			...
			tbb::parallel_pipeline(
				...
				tbb::make_filter<InputFrame*, void>(
					tbb::filter_mode::parallel, ExtractTask{ current_voxels, current_clusters, current_cluster_pca })
			);
		});
	}
	...
	void ClusterExtractionPipeline::ExtractTask::operator()(ClusterExtractionPipeline::InputFrame* frame) const {
		if (frame) {
			...
			// seperate out the pointclouds and calculate bounding boxes for each cluster
			tbb::concurrent_vector<pcl::PointCloud<pcl::PointXYZ>::Ptr> cluster_point_clouds;
			tbb::concurrent_vector<pc::AABB> cluster_bounds;

			tbb::parallel_for(tbb::blocked_range<size_t>(0, cluster_indices.size()),
				[&cluster_indices, &voxelised_cloud, &cluster_point_clouds, &cluster_bounds]
			...
			for (int i = 0; i < cluster_bounds.size(); ++i) {
				...
				static constexpr auto matchClusters = [](const pc::AABB &a, const pc::AABB &b, float tolerance) {
				...
				for (auto& existing_cluster : *existing_clusters) {
					if (matchClusters(existing_cluster.bounding_box, new_bound, config.cluster_match_tolerance)) {
			...
			for (const auto& existing_cluster : *existing_clusters) {
				if (matched_existing_clusters.find(&existing_cluster) == matched_existing_clusters.end()) {
					// this existing cluster has not been matched, check its timeout
					if (now - existing_cluster.last_seen_time <= milliseconds(config.cluster_timeout_ms)) {
			...
			std::transform(updated_clusters.begin(), updated_clusters.end(), output_bounds.begin(),
			...
			if (config.calculate_pca) {
				...
				tbb::parallel_for(tbb::blocked_range<size_t>(0, updated_clusters.size()),
				  [&updated_clusters, &cluster_pca_results] (const tbb::blocked_range<size_t>& r) {
						for (size_t i = r.begin(); i != r.end(); ++i) {
							...
							// - this is so that we ignore up-down and just get horizontal span
							static const Eigen::Vector3f up_axis(0, 1, 0);
							int up_down_index = 0;
							float max_dot_product = std::abs(eigenvectors.col(0).dot(up_axis));
							for (int i = 1; i < 3; ++i)
							{
							...
							if (eigenvalues(horizontal_axis_index) < eigenvalues(3 - up_down_index - horizontal_axis_index)) {
								horizontal_axis_index = 3 - up_down_index - horizontal_axis_index;
							}
							// select the corresponding eigenvector
							Eigen::Vector3f horizontal_axis = eigenvectors.col(horizontal_axis_index);
							...
							// project points onto the selected horizontal axis and find min/max projections
							// - set up the result variables for this:
							static constexpr auto max_f = std::numeric_limits<float>::max();
							static constexpr auto min_f = std::numeric_limits<float>::lowest();
							float min_projection = max_f;
							float max_projection = min_f;
							Eigen::Vector3f min_point, max_point;
							// also find min/max extremes for each axis (unprojected, aabb aligned)
							// - set up these results:
							std::array<PCAResult::minMax, 3> axis_extremes;
							for (int axis = 0; axis < 3; axis++) {
								axis_extremes[axis] = {PCAResult::float3{max_f, max_f, max_f}, PCAResult::float3{min_f, min_f, min_f}};
							}
							...
							for (const auto &point : cluster_cloud->points)
							{
								...
								float projection = (p - centroid.head<3>()).dot(horizontal_axis);
							...
							cluster_pca_results.push_back(PCAResult{
								...
								.principal_axis_span = {
									std::array<float, 3>{min_point.x(), min_point.y(), min_point.z()},
									std::array<float, 3>{max_point.x(), max_point.y(), max_point.z()}},
				...
				auto output_pca_results = std::make_shared<std::vector<PCAResult>>(
					std::move_iterator(cluster_pca_results.begin()), std::move_iterator(cluster_pca_results.end()));

				// // auto output_pca_results = std::make_shared<std::vector<PCAResult>>(principal_spans.size());
				// // std::copy(cluster_pca_results.begin(), cluster_pca_results.end(), output_pca_results->begin());
				// // current_cluster_pca.store(output_pca_results);

				if (config.publish_pca) {
					// publish the structure as a flat array, but also as individual channels
					...
					using FlattenedPCAResult = decltype(std::declval<PCAResult>().flattened());
					std::vector<FlattenedPCAResult> flattened_pca_results(cluster_count);
					std::transform(output_pca_results->begin(), output_pca_results->end(), flattened_pca_results.begin(),
								   [](const auto &result) { return result.flattened(); });
					publisher::publish_all("flattened", flattened_pca_results, { id_string, "pca" });
    		...
    		current_clusters.store(std::make_shared<std::vector<Cluster>>(std::move(updated_clusters)));
