...
bool vector_table(const std::string &label, std::array<T, N> &vec, T min, T max, T reset_value) {
	...
	ImGui::PushStyleVar(ImGuiStyleVar_ItemSpacing, {0, 0});
        ...
        if constexpr (std::is_same_v<T, float>) {
            ImGui::SliderFloat((std::string("##") + label + "." + dimensions[i]).c_str(), &vec[i], min, max);
        } else if constexpr (std::is_same_v<T, int>) {
            ImGui::SliderInt((std::string("##") + label + "." + dimensions[i]).c_str(), &vec[i], min, max);
        }
        ImGui::Dummy({0, 5});
	ImGui::PopStyleVar();
