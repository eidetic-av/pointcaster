...
void drawColorControl(std::string label, rs2_option option, float* value, rs2::option_range range) {
...
void Rs2Device::drawDeviceSpecificControls() {
	// TODO can all of the following be encapsulated in a function that works for all settings?

	// check if the boundaries have been set from device values before using them
	if (!driver) {
		driver = dynamic_cast<Rs2Driver*>(_driver.get());
		exposure_range = driver->getColorSensor().get_option_range(RS2_OPTION_EXPOSURE);
		gain_range = driver->getColorSensor().get_option_range(RS2_OPTION_GAIN);
		brightness_range = driver->getColorSensor().get_option_range(RS2_OPTION_BRIGHTNESS);
		contrast_range = driver->getColorSensor().get_option_range(RS2_OPTION_CONTRAST);
	}

	drawColorControl(label("Exposure"), RS2_OPTION_EXPOSURE, &current_exposure, exposure_range);
	drawColorControl(label("Gain"), RS2_OPTION_GAIN, &current_gain, gain_range);
	drawColorControl(label("Brightness"), RS2_OPTION_BRIGHTNESS, &current_brightness, brightness_range);
	drawColorControl(label("Contrast"), RS2_OPTION_CONTRAST, &current_contrast, contrast_range);
}
