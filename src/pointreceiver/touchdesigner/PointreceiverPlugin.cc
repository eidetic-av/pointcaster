...
to_signal(pointreceiver_message_type t) noexcept {
  ...
  default:
    return PointreceiverMessageSignal::Unknown;
  }
}
...
struct ConnectionState {
  ...
  // ----------------------------------------
  // Multi-channel pointcloud socket handling
  // ----------------------------------------
  ...
  struct ChannelState {
    ...
    std::uint64_t frames_received = 0; // worker-only

    void ensure_pool(std::uint32_t cap, std::uint32_t pool_size) {
      if (capacity_points == cap && frame_pool.size() == pool_size) return;
      capacity_points = cap;
      frame_pool.assign(pool_size, nullptr);
      for (auto &frame : frame_pool) {
        frame = std::make_shared<PointreceiverPointCloudFrame>();
        frame->positions_xyz.resize(cap * 3u);
        frame->colours_rgba.resize(cap * 4u);
    ...
    std::shared_ptr<PointreceiverPointCloudFrame> acquire_writable() {
      const auto n = frame_pool.size();
      for (std::size_t offset = 0; offset < n; ++offset) {
        const auto i = (cursor + offset) % n;
        if (frame_pool[i] && frame_pool[i].use_count() == 1) {
          cursor = (i + 1) % n;
          return frame_pool[i];
        }
      }
  ...
  std::shared_mutex channels_access;
  std::unordered_map<std::string, std::unique_ptr<ChannelState>>
      channels; // node ptrs stable

  std::mutex channel_subscription_mutex;
  std::unordered_map<std::string, int> channel_refcounts;

  ChannelState &channel_for(const std::string &address) { // worker thread
    {
      std::shared_lock r(channels_access);
      if (auto it = channels.find(address); it != channels.end())
        return *it->second;
    }
    std::unique_lock w(channels_access);
    auto &channel = channels[address];
    if (!channel) channel = std::make_unique<ChannelState>();
    return *channel;
  }

  void add_channel_ref(const std::string &address) {
    std::lock_guard lock(channel_subscription_mutex);
    if (++channel_refcounts[address] == 1 && pointreceiver_ctx)
      pointreceiver_subscribe_to_point_cloud(pointreceiver_ctx,
                                             address.c_str());
  }
  void remove_channel_ref(const std::string &address) {
    std::lock_guard lock(channel_subscription_mutex);
    if (auto it = channel_refcounts.find(address);
        it != channel_refcounts.end() && --it->second == 0) {
      channel_refcounts.erase(it);
      if (pointreceiver_ctx)
        pointreceiver_unsubscribe_from_point_cloud(pointreceiver_ctx,
                                                   address.c_str());
    }
  }
  void replay_channel_subscriptions() { // after reconnect rebuilds the context
    std::lock_guard lock(channel_subscription_mutex);
    for (const auto &[address, count] : channel_refcounts)
      if (count > 0)
        pointreceiver_subscribe_to_point_cloud(pointreceiver_ctx,
                                               address.c_str());
  }
  ...
  void reconnect() {
    ...
    replay_channel_subscriptions();
    {
      std::lock_guard lock(channels_access);
      for (auto &[_, channel] : channels) {
        channel->published.store({}, std::memory_order_release);
        channel->sequence.store(0, std::memory_order_release);
      }
    }
  }
  ...
  void pointcloud_worker_main(std::stop_token stop_token) {
    ...
    while (!stop_token.stop_requested()) {
      ...
      // TODO this is for temporary centroid calculation
      float sx = 0;
      float sy = 0;
      float sz = 0;
      ...
      for (std::uint32_t i = 0; i < num_points; ++i) {
        ...
        pos_out[i * 3u + 0u] = static_cast<float>(p.x) * mm_to_m;
        pos_out[i * 3u + 1u] = static_cast<float>(p.y) * mm_to_m;
        pos_out[i * 3u + 2u] = static_cast<float>(p.z) * mm_to_m;
      ...
      writable_frame->centroid_xyz[0] = sx / static_cast<float>(num_points);
      writable_frame->centroid_xyz[1] = sy / static_cast<float>(num_points);
      writable_frame->centroid_xyz[2] = sz / static_cast<float>(num_points);

      const std::uint64_t seq = ++channel.frames_received;
      writable_frame->sequence = seq;
      writable_frame->num_points = num_points;
      channel.published.store(
          std::static_pointer_cast<const PointreceiverPointCloudFrame>(
              writable_frame),
          std::memory_order_release);
      channel.sequence.store(seq, std::memory_order_release);

      const float ms = std::chrono::duration<float, std::milli>(clock::now() -
                                                                loop_start_time)
                           .count();
  ...
  convert_message(const pointreceiver_sync_message &msg) {
    ...
    default:
      return PointreceiverMessageValue{std::monostate{}};
    }
  }
...
PointreceiverChannelSubscription::PointreceiverChannelSubscription(
    std::shared_ptr<ConnectionState> state, std::string address)
    : _state(std::move(state)), _address(std::move(address)) {
  _state->add_channel_ref(_address);
}

PointreceiverChannelSubscription::~PointreceiverChannelSubscription() {
  _state->remove_channel_ref(_address);
}

bool PointreceiverChannelSubscription::try_get_latest_frame(
    std::uint64_t last_seen_sequence,
    PointreceiverFrameView &out_view) const noexcept {
  std::shared_ptr<const PointreceiverPointCloudFrame> frame;
  {
    std::shared_lock r(_state->channels_access);
    auto it = _state->channels.find(_address);
    if (it == _state->channels.end()) return false;
    frame = it->second->published.load(std::memory_order_acquire);
  }
  ...
  // TODO
  out_view.centroid_xyz = out_view._frame->centroid_xyz;
...
// ---------------------------
// Handle: channel subscription + discovery
// ---------------------------

std::shared_ptr<PointreceiverChannelSubscription>
PointreceiverConnectionHandle::subscribe(const std::string &address) {
  return std::make_shared<PointreceiverChannelSubscription>(_state, address);
}

std::vector<std::string>
PointreceiverConnectionHandle::known_point_cloud_addresses() const {
  std::vector<std::string> result;
  if (!_state) return result;

  // hold the reconnect lock so the context can't be swapped/destroyed
  // mid-enumeration
  std::lock_guard reconnect_lock(_state->_reconnect_mutex);
...
void PointreceiverConnectionHandle::reconnect() {
  if (_state) _state->reconnect();
}
