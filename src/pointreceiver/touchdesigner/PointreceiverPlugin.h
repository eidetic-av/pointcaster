...
class PointreceiverChannelSubscription;
...
struct PointreceiverGlobalConfig {
  std::atomic<std::uint32_t> max_points_capacity{5'000'000};
...
struct PointreceiverPointCloudFrame {
  ...
  // TODO this is a temporary place to calculate the centroid
  // really the point cloud should be sending its bounds over the wire too
  std::array<float, 3> centroid_xyz{0.0f, 0.0f, 0.0f};
...
struct PointreceiverFrameView {
  ...
  std::array<float, 3> centroid_xyz;
...
private:
  ...
  friend class PointreceiverChannelSubscription;
...
public:
  ...
  std::vector<std::string> known_point_cloud_addresses() const;
...
class PointreceiverChannelSubscription {
public:
  PointreceiverChannelSubscription(std::shared_ptr<ConnectionState> state,
                                   std::string address);
  ~PointreceiverChannelSubscription();

  PointreceiverChannelSubscription(const PointreceiverChannelSubscription &) =
      delete;
  PointreceiverChannelSubscription &
  operator=(const PointreceiverChannelSubscription &) = delete;
