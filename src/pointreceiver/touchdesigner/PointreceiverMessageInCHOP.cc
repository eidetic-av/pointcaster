      ...
      [&](const auto &payload) {
        ...
        if constexpr (std::is_same_v<T, std::monostate>) {
          ...
          for (int i = 0; i < n; ++i) {
            ...
            const float minx = a.min[0] * mm_to_m;
            const float miny = a.min[1] * mm_to_m;
            const float minz = a.min[2] * mm_to_m;
            const float maxx = a.max[0] * mm_to_m;
            const float maxy = a.max[1] * mm_to_m;
            const float maxz = a.max[2] * mm_to_m;
...
void PointreceiverMessageInCHOP::getInfoCHOPChan(int32_t index,
  ...
  if (index == 1) {
    chan->name->setString("messages_received");
    chan->value = static_cast<float>(stats.messages_received);
  } else if (index == 2) {
    chan->name->setString("message_worker_last_ms");
    chan->value = stats.message_worker_last_ms;
  } else if (index == 3) {
    chan->name->setString("message_worker_avg_ms");
    chan->value = stats.message_worker_avg_ms;
  } else if (index == 4) {
    chan->name->setString("selected_revision");
    chan->value = static_cast<float>(_selected_revision_cached);
  } else if (index == 5) {
    chan->name->setString("known_ids_count");
