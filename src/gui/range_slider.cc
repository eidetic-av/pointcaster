...
// Taken from:
// https://github.com/wasikuss/imgui/commit/a50515ace6d9a62ebcd69817f1da927d31c39bb1
...
// extern float RoundScalarWithFormatFloat(const char* format, ImGuiDataType
// data_type, float v);
...
bool RangeSliderBehavior(const ImRect &frame_bb, ImGuiID id, float *v1,
                         float *v2, float v_min, float v_max, float power,
  ...
  window->DrawList->AddRectFilled(grab_bb1.Min, grab_bb1.Max,
  ...
  if (is_horizontal)
    ...
    grab_bb2 = ImRect(
        ImVec2(frame_bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f),
        ImVec2(frame_bb.Max.x - grab_padding, grab_pos + grab_sz * 0.5f));
  window->DrawList->AddRectFilled(grab_bb2.Min, grab_bb2.Max,
                                  GetColorU32(g.ActiveId == id
                                                  ? ImGuiCol_SliderGrabActive
                                                  : ImGuiCol_SliderGrab),
                                  style.GrabRounding);
  ...
  window->DrawList->AddRectFilled(connector.Min, connector.Max,
...
bool RangeSliderFloat(const char *label, float *v1, float *v2, float v_min,
  ...
  const bool value_changed = RangeSliderBehavior(
  ...
  const char *value_buf_end =
      value_buf + ImFormatString(value_buf, IM_ARRAYSIZE(value_buf),
                                 display_format, *v1, *v2);
